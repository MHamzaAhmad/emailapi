...
var (
	...
	// Validation (3xx)
	ErrInvalidArgument      = NewError(v1.ErrorCode_ERROR_CODE_INVALID_ARGUMENT, "request contains invalid or malformed data")
	ErrMissingRequiredField = NewError(v1.ErrorCode_ERROR_CODE_MISSING_REQUIRED_FIELD, "a required field is missing from the request")
	ErrInvalidEmailSyntax   = NewError(v1.ErrorCode_ERROR_CODE_INVALID_EMAIL_SYNTAX, "email address has invalid syntax")
	ErrNoMXRecords          = NewError(v1.ErrorCode_ERROR_CODE_NO_MX_RECORDS, "recipient domain cannot receive email (no MX records)")
	ErrEmailTypoDetected    = NewError(v1.ErrorCode_ERROR_CODE_EMAIL_TYPO_DETECTED, "possible typo detected in email address")
	ErrUnsafeURL            = NewError(v1.ErrorCode_ERROR_CODE_UNSAFE_URL, "URL in email body flagged as unsafe")
	ErrEmailSuppressed      = NewError(v1.ErrorCode_ERROR_CODE_EMAIL_SUPPRESSED, "recipient email is on suppression list")
	ErrSandboxRestriction   = NewError(v1.ErrorCode_ERROR_CODE_SANDBOX_RESTRICTION, "sandbox mode restricts recipients to your verified email")
	...
	ErrDomainNotOwned       = NewError(v1.ErrorCode_ERROR_CODE_DOMAIN_NOT_OWNED, "you have not registered this domain for sending")
	ErrDomainNotVerified    = NewError(v1.ErrorCode_ERROR_CODE_DOMAIN_NOT_VERIFIED, "domain DNS verification is incomplete")
	ErrDomainDNSMismatch    = NewError(v1.ErrorCode_ERROR_CODE_DOMAIN_DNS_MISMATCH, "DNS record does not match expected value")
	ErrDomainVerifyCooldown = NewError(v1.ErrorCode_ERROR_CODE_DOMAIN_VERIFY_COOLDOWN, "please wait 30 seconds before retrying verification")
